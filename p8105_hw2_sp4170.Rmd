...
* Discuss important steps in the import process and relevant features of the dataset.  
  ...
  * need to use `col_names = TRUE` to make the original first row to be the columns name.
  * need to use `janitor::clean_names()` to make sure column names are all in snake_case format.
  * need to use `filter()` to keep the rows with age_at_onset is not of null value ".".
...
```{r}
x_df = filter(mci_baseline_original, sex == 1)
y_df = filter(mci_baseline_original, sex == 1 & apoe4 == 1)
```
  * the proportion of women in the study are APOE4 carriers is `r round(nrow(y_df) / nrow(x_df), digits = 4)`, which is the same as 30.15%.
