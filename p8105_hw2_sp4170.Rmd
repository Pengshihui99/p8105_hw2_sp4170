...
```
* based on the anti_join process, there are `r nrow(amyloid_only_df)` participants who are only appear in the amyloid dataset but not appear in the baseline dataset.
...
```
* there are `r nrow(baseline_amyloid_both_df)` participants appear in both datasets. the combined data set has `r nrow(baseline_amyloid_both_df)` rows and `r ncol(baseline_amyloid_both_df)` cols. the names of variables are `r colnames(baseline_amyloid_both_df)`.

```{r}
baseline_amyloid_pivot_both_df = 
  inner_join(mci_amyloid_pivot_df, mci_baseline, by = "id")
```
* there are `r nrow(baseline_amyloid_pivot_both_df)` participants appear in both datasets. the combined data set has `r nrow(baseline_amyloid_pivot_both_df)` rows and `r ncol(baseline_amyloid_pivot_both_df)` cols. the names of variables are `r colnames(baseline_amyloid_pivot_both_df)`.
