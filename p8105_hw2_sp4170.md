...
- need to skip the first row with `skip = 1` because variable names have
  taken a whole row in the original file.
...
- the average baseline age is 65.0286, after we filter the original data
  set based on the inclusion criteria “no MCI at base line”.
...
- when using pivot:
  ...
  - we want to put the time variable which are included in the columns
    ...
    column as “follow_up_time” using `names_to = "follow_up_time"`. Then
    all the values that corresponding to the columns time_2 ~ time_8 are
    moved to a new column named “amyloid” using `values_to = "amyloid"`.
...
- based on the anti_join process, there are 8 participants who are only
  appear in the baseline dataset but not appear in the amyloid dataset.
...
- based on the anti_join process, there are 64 participants who are only
  appear in the amyloid dataset but not appear in the baseline dataset.
...
describe the resulting dataset.* Since i have 2 datasets for amyloid
data, i will try them both.
...
- there are 471 participants appear in both datasets “mci_amyloid_df”
  and “mci_baseline”. the combined data set has 471 rows and 11 cols.
  the names of variables are id, baseline, time_2, time_4, time_6,
  time_8, current_age, sex, education, apoe4, age_at_onset.
...
- there are 1884 participants appear in both datasets
  “mci_amyloid_pivot_df” and “mci_baseline”. the combined data set has
  1884 rows and 9 cols. the names of variables are id, baseline,
  follow_up_time, amyloid, current_age, sex, education, apoe4,
  age_at_onset.

## export final data set

*export the result as a CSV to your data directory.* export the
combining dataset for “mci_amyloid_pivot_df” and “mci_baseline”
