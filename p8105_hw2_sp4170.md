...
- Discuss important steps in the import process and relevant features of
  ...
  - need to use `col_names = TRUE` to make the original first row to be
    the columns name.
  - need to use `janitor::clean_names()` to make sure column names are
    all in snake_case format.
  - need to use `filter()` to keep the rows with age_at_onset is not of
    null value “.”.
...
- there are 483 participants recruited in the study. Of these
  participants, there are 97 of participants who developed MCI.

- What is the average baseline age?

  - the average baseline age is 65.61
...
- the proportion of women in the study are APOE4 carriers is 0.3015,
  which is the same as 30.15%.
